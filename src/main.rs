...
fn hosts_builder(path: String) -> Vec<String> {
    ...
    info!("config len: {}", hosts.len());
    hosts
}

fn connect_host(host: String) -> String
{
    info!("Started processing {}", &host);
    let tcp = match TcpStream::connect(&host) {
        ...
        Err(e) => {
            ...
            return format!("Failed connected to {}:{}", &host, e);
        }
    };
    ...
    let mut sess = match Session::new() {
        ...
        Err(e) => {
            ...
            return format!("Failed making session on {}:{}", &host, e);
        }
    };
    ...
    match sess.handshake() {
        ...
        Err(e) => {
            ...
            return format!("Failed establishing handshake with {}:{}", &host, e);
        }
    }
    const LEN: usize = 10;
    ...
    // debug!("Finished processing for {}", &host);
    format!("Done for: {}", &host)
}
...
fn process_hosts(hosts: Vec<String>) {
    ...
    let n_workers = 8;
    debug!("Workers number: {}", n_workers);
    let pool = Pool::<Fool>::new(n_workers);
    let (tx, rx) = channel();
    for host in &hosts {
        let host_name = host.clone();
        debug!("Started processing {}", &host_name);
